...
for INPUT_FILE in $BIN_DIR/*.riscv; do
    ...
    $DRO $RESET_VECTOR_OPT $OPT --stf_trace traces/"$base_name".zstf "$INPUT_FILE"
    
    if [ -f "$GOLDEN_DIR/$base_name".zstf ]; then
        echo "Comparing binary trace files for $base_name..."
        if diff traces/"$base_name".zstf "$GOLDEN_DIR/$base_name".zstf > /dev/null; then
